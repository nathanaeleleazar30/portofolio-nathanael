...
export default function Home() {
  const hardSkills = [
    ...
    { name: 'Multimedia & Broadcasting (ProPresenter, OBS)', level: '85%', icon: '🎥' },
    { name: 'Graphic Design (Photoshop, Canva)', level: '85%', icon: '🖼️' },
    { name: 'ReactJS, Next.js & Tailwind CSS', level: '75%', icon: '💻' },
  ];
  ...
  const projectData = [
    ...
    },
    {
      ...
      description: 'Menginisiasi dan mengeksekusi berbagai wadah minat bakat olahraga bagi mahasiswa Fakultas Vokasi UB di bawah naungan Kementerian SENORA.',
      gradient: 'from-orange-500 to-rose-600'
    },
    {
      title: 'STIVOFEST 2026',
      tech: 'Festival Olahraga | Malang Raya',
      description: ' Menginisasi event olahraga yang menjadi wadah kompetisi dan kolaborasi antar kampus dan SMA/SMKdi Malang Raya, dengan target 500+ peserta.',
      gradient: 'from-amber-600 to-amber-900'
    }
  ];

  const animationFadeUp = "transition-all duration-700 delay-100 group-hover:scale-105 group-hover:-translate-y-1 transform hover:-translate-y-2 hover:shadow-xl";

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-indigo-100 selection:text-indigo-900 font-sans relative">
      <div className="bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-10 pointer-events-none absolute inset-0 z-0"></div>

      {/* === NAVBAR MODERN ==== */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-xl border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center font-black text-white text-lg">N</div>
            <span className="text-xl font-extrabold tracking-tight text-slate-950">Nathanael</span>
          </div>
          <div className="hidden md:flex items-center gap-6 text-sm font-semibold text-slate-600">
            {['Tentang', 'Pengalaman', 'Proyek', 'Kontak'].map((link, i) => (
              <a key={i} href="#" className="hover:text-blue-600 transition-colors group relative">
                {link}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 md:-mt-12 relative z-20 pb-16">
        <div className="bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl p-6 sm:p-10 lg:p-14 border border-white/60 transition-all duration-300 transform motion-safe:animate-fade-in-up">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end -mt-20 md:-mt-32 mb-10 gap-6 lg:gap-0">
            ...
            <div className="relative group cursor-pointer shrink-0 transition-transform duration-500 hover:scale-105">
              ...
              <div className="absolute bottom-4 right-4 md:bottom-6 md:right-6 z-20 flex h-6 w-6 md:h-8 md:w-8 items-center justify-center">
                ...
                <span className="relative inline-flex rounded-full h-4 w-4 md:h-6 md:w-6 bg-green-500 border-2 border-white"></span>
              </div>
            </div>

            <div className="flex w-full lg:w-auto lg:pb-6">
              <a href="/CV_NATHANAEL.pdf" download className="w-full lg:w-auto px-8 py-3.5 bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 hover:from-blue-700 hover:to-purple-900 text-white text-sm font-extrabold rounded-2xl shadow-xl hover:shadow-indigo-500/40 transition-all duration-300 flex items-center justify-center gap-2 transform hover:-translate-y-1.5 active:scale-95">
          ...
          {/* === IDENTITAS UTAMA ==== */}
          <div className="border-b border-slate-100 pb-10 text-left">
            <h1 className="text-4xl md:text-5xl font-black text-slate-950 tracking-tight leading-tight">Nathanael Eleazar Handata</h1>
            <p className="text-xl md:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-indigo-700 to-purple-800 mt-2">
              Undergraduate Student of Information Technology
            </p>

            <div className="flex flex-wrap gap-3 mt-6 text-sm font-semibold">
              <span className="flex items-center gap-2 bg-slate-50 text-slate-700 px-4 py-2.5 rounded-xl border border-slate-200">
                Domisili: Palembang
              </span>
              <span className="flex items-center gap-2 bg-slate-50 text-slate-700 px-4 py-2.5 rounded-xl border border-slate-200">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-12">
            ...
            <div className="lg:col-span-1 space-y-10">
              ...
              {/* === CONTACT ME (MODIFIKASI BARU) === */}
              <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-950 p-8 rounded-[2rem] shadow-2xl text-white relative overflow-hidden group">
                  ...
                  {hardSkills.map((skill, index) => (
                    <div key={index} className="group">
                      <div className="flex justify-between mb-2">
                        <span className="text-sm font-bold text-slate-700">{skill.icon} {skill.name}</span>
                        <span className="text-sm font-black text-indigo-700">{skill.level}</span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden border border-slate-200/50">
                        <div 
                          className="bg-gradient-to-r from-blue-600 via-indigo-700 to-purple-800 h-2.5 rounded-full transition-all duration-1000 ease-out delay-300 transform scale-x-0 group-hover:scale-x-100 origin-left" 
            ...
            <div className="lg:col-span-2 space-y-14">
              ...
              <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-500">
                <h2 className="text-2xl font-black text-slate-950 mb-6 flex items-center gap-3">
                  Tentang Diri
                </h2>
                <div className="text-slate-700 leading-relaxed text-justify text-sm space-y-5 font-medium">
                  <p>
              ...
              <div>
                ...
                <div className="space-y-6 relative before:absolute before:inset-y-0 before:left-[18px] before:w-0.5 before:bg-gradient-to-b before:from-blue-600 before:to-slate-200 before:rounded-full">
                  {experienceData.map((exp, i) => (
                    <div key={i} className="relative pl-14 group">
                      <div className="absolute left-0 top-1 w-9 h-9 bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 rounded-full border-4 border-white shadow-md flex items-center justify-center transition-transform group-hover:scale-110">
                        <div className="w-2.5 h-2.5 bg-white rounded-full"></div>
                      </div>
                      <div className={`bg-white p-7 rounded-[2rem] border border-slate-100 shadow-sm ${animationFadeUp}`}>
                        <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-2 gap-2">
                          <h4 className="font-black text-slate-950 text-xl tracking-tight leading-snug">{exp.role}</h4>
                          <span className="inline-block px-3 py-1 bg-blue-50 text-blue-800 text-xs font-black rounded-full w-fit">{exp.time}</span>
                        </div>
                        <p className="text-sm font-bold text-indigo-700 mb-3">{exp.organization}</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {projectData.map((project, i) => (
                    <div key={i} className={`bg-white rounded-[2rem] border border-slate-100 shadow-sm group overflow-hidden ${animationFadeUp}`}>
                      ...
                      <div className="p-6">
                        <p className="text-xs font-bold text-indigo-700 mb-2 uppercase tracking-wider">{project.tech}</p>
      ...
      {/* === FOOTER PROFESSIONAL ==== */}
      <footer className="bg-slate-950 text-white mt-10 pt-16 pb-12 border-t-8 border-indigo-700 relative z-0">
